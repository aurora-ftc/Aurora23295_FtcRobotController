# AURORA 2025 DECODE CODE REPO

[Check Out our website!](https://aurora-ftc.github.io)

## Resources for our new coders:

You can read through the code samples from FTC themselves. Visit\
```FtcRobotController > java > org.firstinspires.ftc.robotcontroller```\
and read through some of the internal/external samples or\
```Teamcode > java > org.firstinspires.ftc.teamcode```\
to see our code this season.

My top recommendations: 

* [Brogan M. Pratt on Youtube](https://www.youtube.com/@BroganMPratt)
* ["Learn Java for Robotics"](https://www.youtube.com/watch?v=_ZIYtNadJBo&list=PLRHdgFNRLyaPiZ5rvINwMmGMHEIL9usla)
* [Gm0 manual](https://gm0.org/en/latest/docs/software/getting-started/index.html)
* [FTC docs](https://ftc-docs.firstinspires.org/en/latest/programming_resources/android_studio_java/Android-Studio-Tutorial.html)
* [Artemis Robotics on YouTube](https://www.youtube.com/watch?v=PpD9UWXfEag&list=PLnyefHqkc2K-gQD4hO5x3IcANuDuNMKDP)
* [Or visit the FTC Discord!](https://discord.gg/sFkYKHd4)
